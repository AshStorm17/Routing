...
if __name__ == '__main__':
    ...
    info('*** Assigning public IP to h9-eth0 (NAT public side)\n')
    h9.cmd('ip addr add 172.16.20.10/24 dev h9-eth0')
    h9.cmd('ip link set dev h9-eth0 up')

    info('*** Configuring NAT bridge for h1-h2 (private side) on h9\n')
    h9.cmd('brctl addbr br0')  # Private bridge for h1-h2
    h9.cmd('brctl addif br0 h9-eth1')  # Private link for h1
    h9.cmd('brctl addif br0 h9-eth2')  # Private link for h2
    h9.cmd('ip addr add 10.1.1.1/24 dev br0')
    h9.cmd('ip link set dev br0 up')

    info('*** Configuring NAT bridge for h3-h8 (private side) on h9\n')
    h9.cmd('brctl addbr br1')  # Private bridge for h3-h8
    h9.cmd('brctl addif br1 h9-eth3')  # Private link for h3
    h9.cmd('brctl addif br1 h9-eth4')  # Private link for h4
    h9.cmd('brctl addif br1 h9-eth5')  # Private link for h5
    h9.cmd('brctl addif br1 h9-eth6')  # Private link for h6
    h9.cmd('brctl addif br1 h9-eth7')  # Private link for h7
    h9.cmd('brctl addif br1 h9-eth8')  # Private link for h8
    h9.cmd('ip addr add 10.0.0.1/24 dev br1')
    h9.cmd('ip link set dev br1 up')

    info('*** Enabling NAT\n')
    h9.cmd('sysctl -w net.ipv4.ip_forward=1')
    h9.cmd('iptables -t nat -A POSTROUTING -o h9-eth0 -j MASQUERADE')

    # Set up forwarding rules for bridges
    ...
    h9.cmd('iptables -A FORWARD -i h9-eth0 -o br1 -m state --state RELATED,ESTABLISHED -j ACCEPT')
    h9.cmd('iptables -A FORWARD -i br1 -o h9-eth0 -j ACCEPT')

    h9.cmd('iptables -A FORWARD -i br0 -o br1 -j ACCEPT')
    h9.cmd('iptables -A FORWARD -i br1 -o br0 -j ACCEPT')
    h9.cmd('iptables -A FORWARD -i br0 -o h9-eth0 -j ACCEPT')
    h9.cmd('iptables -A FORWARD -i br1 -o h9-eth0 -j ACCEPT')

    # Add routing (optional but explicit)
    h9.cmd('ip route add 10.0.0.0/24 dev br1')
    h9.cmd('ip route add 10.1.1.0/24 dev br0')

    print("Waiting 60 seconds for STP convergence...")
    sleep(60)

    # Ensure Spanning Tree Protocol convergence
    verify_stp(net)

    # Run network tests like ping and iperf
    ping_server()
    iperf_test()

    # Stop the network after the tests
    net.stop()
